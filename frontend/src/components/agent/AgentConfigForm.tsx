  ...
  return (
    <div className="h-full w-full bg-white flex flex-col overflow-hidden">
      <div className="flex items-center justify-between p-4 border-b border-slate-200">
        <h3 className="text-base font-semibold text-slate-800">配置 {agent.name}</h3>
      ...
      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-4 space-y-5">
        <div className="space-y-3">
          <h4 className="text-sm font-medium text-slate-700 border-b pb-1 border-slate-100">基础信息</h4>
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Agent名称</label>
            <input
              type="text"
              value={form.name}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">头像URL</label>
            <input
              type="text"
              value={form.avatar}
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-4 h-4"
            />
            <label htmlFor="enabled" className="text-sm text-slate-700">启用Agent</label>
          </div>
        </div>

        <div className="space-y-3">
          <h4 className="text-sm font-medium text-slate-700 border-b pb-1 border-slate-100">Prompt配置</h4>
          <div>
        ...
        <div className="space-y-3">
          <h4 className="text-sm font-medium text-slate-700 border-b pb-1 border-slate-100">模型配置</h4>
          <div>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">Temperature: {form.modelConfig.temperature.toFixed(1)}</label>
            <input
              type="range"
              min="0"
              max="2"
        ...
        {form.tools.length > 0 && (
          <div className="space-y-3">
            <h4 className="text-sm font-medium text-slate-700 border-b pb-1 border-slate-100">工具配置</h4>
              ...
              {form.tools.map(tool => (
                <div key={tool.id} className="flex items-center gap-2">
                  <input
                    ...
                    className="w-4 h-4"
                  />
                  <label htmlFor={`tool-${tool.id}`} className="text-sm text-slate-700">
                    {tool.name}
                    <span className="text-xs text-slate-500 ml-1">({tool.description})</span>
                  </label>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-3">
          <h4 className="text-sm font-medium text-slate-700 border-b pb-1 border-slate-100">权限配置</h4>
          <div className="space-y-2">
            {[
              ...
              <div key={p.key} className="flex items-center gap-2">
                <input
                  ...
                  className="w-4 h-4"
                />
                <label htmlFor={`perm-${p.key}`} className="text-sm text-slate-700">{p.label}</label>
              </div>
            ))}
          </div>
        </div>

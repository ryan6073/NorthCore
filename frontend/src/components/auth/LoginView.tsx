...
import { Sparkles, User, Mail, ShieldAlert } from 'lucide-react';
...
export const LoginView: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      login(name.trim(), email.trim(), selectedAvatar);
      setLoading(false);
    }, 1200);
  };
  ...
  const handleQuickLogin = () => {
    ...
    setTimeout(() => {
      ...
      const randomEmail = `${Math.floor(Math.random() * 100000)}@norhcore.ai`;
      const randomAvatar = PRESET_AVATARS[Math.floor(Math.random() * PRESET_AVATARS.length)];
      login(randomName, randomEmail, randomAvatar);
      setLoading(false);
    }, 800);
  };
